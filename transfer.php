...
<body style="background-image:url(images/bg-login1.jpg);background-size: 100%">
 ...
 <a class="navbar-brand" href="index.php">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item active">  <a class="nav-link" href="transfer.php">Transfer Money</a></li>
  ...
  <div class="card-body">
      <form method="POST">
          <div class="alert alert-success w-50 mx-auto">
            <h5>New Transfer</h5>
            <input type="text" name="receiverNo" class="form-control " placeholder="Enter Receiver Account number" required>
            <button type="submit" name="get" class="btn btn-primary btn-bloc btn-sm my-1">Get Account Info</button>
          </div>
      </form>
      <?php if (isset($_POST['get'])) 
      {
        $array2 = $con->query("select * from otheraccounts where accountNo = '$_POST[receiverNo]'");
        $array3 = $con->query("select * from userAccounts where accountNo = '$_POST[receiverNo]'");
        {
          ...
          { $row2 = $array2->fetch_assoc();
            echo "<div class='alert alert-success w-50 mx-auto'>
                  <form method='POST'>
                    Account No.
                    <input type='text' value='$row2[accountNo]' name='receiverNo' class='form-control ' readonly required>
                    Account Holder Name.
           ...
           $row2 = $array3->fetch_assoc();
            echo "<div class='alert alert-success w-50 mx-auto'>
                  <form method='POST'>
                    ...
                    Account Holder Bank Name.
                    <input type='text' class='form-control' value='X - Bank' readonly required>
                    Enter Amount for tranfer.
                    <input type='number' name='amount' class='form-control' min='1' max='$userData[balance]' required>
            ...
            echo "<div class='alert alert-success w-50 mx-auto'>Account No. $_POST[receiverNo] Does not exist</div>";
        }
      } 
      ?>
    <br>
    ...
    {
      echo '<script>console.log("transferSelf initiated")</script>';
      $amount = $_POST['amount'];
      setBalance($amount,'debit',$userData['accountNo']);
      setBalance($amount,'credit',$_POST['receiverNo']);
      makeTransaction('transfer',$amount,$_POST['receiverNo']);
      echo "<script>alert('Transfer Successfull');window.location.href='transfer.php'</script>";
    }
    if (isset($_POST['transfer']))
    {
       echo '<script>console.log("transfer initiated")</script>';
      $amount = $_POST['amount'];
      setBalance($amount,'debit',$userData['accountNo']);
      makeTransaction('transfer',$amount,$_POST['receiverNo']);
      echo "<script>alert('Transfer Successfull');window.location.href='transfer.php'</script>";
